...
public object JsMetaFileUtils {
    ...
    private val DEFAULT_PACKAGE_METAFILE_NAME = PackageClassUtils.DEFAULT_PACKAGE_CLASS_NAME + "." + KotlinJavascriptMetaFileType.INSTANCE.getDefaultExtension()

    private fun getClassFqName(relPath: String): FqName {
        val pathToFile = relPath.substringAfter(VfsUtilCore.VFS_SEPARATOR_CHAR)

        return FqName(pathToFile.substringBeforeLast('.').replace('/', '.'))
    }
    ...
    private fun getPackageFqName(relPath: String): FqName {
        val pathToFile = relPath.substringAfter(VfsUtilCore.VFS_SEPARATOR_CHAR)
        if (pathToFile == DEFAULT_PACKAGE_METAFILE_NAME) return FqName.ROOT
